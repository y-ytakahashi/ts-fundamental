...
### ドメイン拡張: 先生ロール（Week2 Day4で追加）

**背景**: 既存スキーマ（Student/Problem/Answer）はFKが2本しかなく、しかもどちらも1:Nのみで、M:N（中間テーブル）を練習する機会が一つもないことに気づいた。Week2 Day4（Repositoryパターン導入の最終日）に、新しいアクター「先生（Teacher）」を追加し、意図的にM:N関係を持ち込むスキーマ拡張を組み込む。

**新しいアクター**: 先生（Teacher）— 問題を作成する（複数の先生が共著できる）、問題を編集する、回答を採点する

**追加テーブルと設計判断（確定分）**:

| テーブル | 主キー | 設計判断の理由 |
|---|---|---|
| `Teacher` | 独立ID | 他のエンティティ（Student, Problem）と統一 |
| `ProblemAuthor`（先生:問題のM:N、作成記録） | 複合キー`(problemId, teacherId)` | 「誰が作成者か」は1回きりの固定事実で、同じペアの重複を防ぎたい。複合キー自体が一意性を構造的に保証するので追加のUNIQUE制約が不要。**問題の更新（編集）ではこのテーブルに触らない**（作成と編集は別イベント） |
| `ProblemEdit`（編集履歴） | 独立ID、UNIQUE制約なし | 同じ先生が同じ問題を何度でも編集し直せる（履歴として残したい）ため、複合キーでは表現できない。編集1回＝1行 |
| `AnswerGrade`（採点履歴、先生:回答のM:N） | 独立ID、UNIQUE制約なし | 同じ先生が同じ回答を何度でも採点し直せる（再採点の履歴を残したい）ため、`ProblemAuthor`とは逆にUNIQUE制約を付けない |

**なぜ複合キー/独立IDが分かれるか**: 「そのペアが1回しか存在しない事実」（作成者）は複合キーが自然（一意性がPKで自動保証される）。「同じペアが繰り返し発生しうるイベント」（編集・採点）は独立IDでないと2回目以降が記録できない。「いつも独立IDを使う」という習慣は『SQLアンチパターン 第2版』4章「IDリクワイアド（とりあえずID）」が指摘する典型的な思考停止パターンなので、テーブルごとに要件を確認してから決めること。

**未確定（実装時に自分で判断する）**:
- `ProblemEdit`・`AnswerGrade`の具体的なカラム構成
- `AnswerGrade`に採点結果（正解/不正解）をどう持たせるか、既存の`Answer.isCorrect`（自動判定）との関係（置き換えるか併用するか）
...
## コードレビューのスタンス（シニアエンジニアとのペアプロ想定）

1ヶ月という短期間でTypeScript/Expressを実務レベルまで引き上げる必要があるため、**Claude Codeは「優しい先生」ではなく「厳しめのシニアエンジニア」としてコードレビューを行うこと。** 具体的には:

1. **甘い採点をしない。** 「動いているから良い」で済ませず、実務のコードレビューで指摘されるレベルの基準で評価する。命名の曖昧さ、型の緩さ（`any`の濫用、戻り値の型省略など）、エラーハンドリングの欠如、テストの不足は、動くコードであっても遠慮なく指摘する
2. **指摘は具体的かつ理由付きで。** 「ここが良くない」で終わらせず、「なぜ問題か」「実務でどう困るか」「どう直すべきか」の3点をセットで伝える。ただし直したコードそのものを渡すのではなく、方向性を示してたあさんに書き直させる（学習方針1と矛盾しないこと）
3. **褒めるところは明確に、かつ簡潔に。** シニアエンジニアが新人を評価する時のように、良い設計判断は端的に認めるが、過度な称賛や励ましのための甘いコメントは避ける
4. **実務基準を基準点にする。** 「学習中だから大目に見る」という前提を極力排除し、「このコードがCOMSBIのプルリクエストに出てきたら、レビュアーはどう指摘するか」を判断基準にする
5. **それでも学習方針とは矛盾させない。** 厳しいレビューであっても、答えを先に渡さない・Week1の汚い設計を急かして直させないという既存の方針は維持する。「厳しさ」は評価の基準の高さであって、答えを教えることの早さではない
...
**Week1着手前の準備**: 『SQLアンチパターン 第2版』（Bill Karwin著、和田卓人・児島修 訳、O'Reilly Japan、2025年7月刊）の目次と各章概要に2〜3時間だけ目を通す。25章の本編アンチパターンに加え、各章末の「ミニ・アンチパターン」、第V部（26〜27章）の外部キー特化のミニ・アンチパターン群も収録されている。深く読み込む必要はなく「こういう罠がある」と存在を知っておくことが目的。Student/Problem/Answerのスキーマ設計（Day1〜3）で、外部キーやインデックスの判断に活きる。**1版（2010年刊）とは25章の内容が異なる（1版のMagic Beans／Active Recordアンチパターンは2版では削除され、スタンダード・オペレーティング・プロシージャに差し替わっている）ので、参照する際は2版に統一すること。**
...
| Day4 | `CreateProblemUsecase`, `DeleteProblemUsecase`をRepository経由に統一 + **先生ロール拡張のスキーマをマイグレーション追加**（`Teacher`, `ProblemAuthor`, `ProblemEdit`, `AnswerGrade`。詳細は「ドメイン拡張: 先生ロール」節を参照） | UC3, UC4 | 5つのUsecase全てが抽象Repositoryに依存している／新テーブルがマイグレーションで作成され、Prisma Studioで確認できる |
...
| Day5 | UC2・UC3・UC4をまとめた管理者用画面 ＋ UC5（生徒用回答一覧）画面を圧縮して実装。**まず生徒一覧を取得後、各生徒ごとに`GET /api/students/:id/answers`を個別にループで呼び出す実装をあえて作る**（フロント版N+1問題）。ブラウザのNetworkタブで生徒N人分のリクエストが直列に発火する様子を確認してから、複数生徒分をまとめて返す新エンドポイント（例: `GET /api/students/answers?ids=1,2,3`）または`Promise.all`による並列化のどちらか（できれば両方）で解消する | UC2, UC3, UC4, UC5 | 一通りの管理者操作と生徒本人の回答確認がフロントから行える／Networkタブで直列リクエストを確認できる／解消後にリクエスト数または体感速度の変化を説明できる |

**Day5のフロントN+1演習について**: Week3 Day2で体験したPrismaのN+1問題と同じ構造がフロント⇔バックエンド間でも起きることを体感させる演習。「N+1はDB特有の知識ではなく、レイヤーをまたいで繰り返し現れる設計原則である」という理解に格上げするのが狙い。Claude Codeは、解消方法として「新エンドポイントでリクエスト数自体を減らす（本質的な解決）」と「`Promise.all`で並列化する（対症療法、リクエスト数は変わらない）」の2つを両方試させ、その違いをたあさんに言語化させること。最初から良い実装を提案せず、Week1の「あえて汚い設計から始める」と同じ流れで、まず悪い実装→気づき→解消の順で進めること。
...
| 『SQLアンチパターン 第2版』（和田卓人・児島修 訳、O'Reilly Japan、2025） | DBスキーマ設計の落とし穴を知る | Week1着手前に2〜3時間（上記参照）。該当章はWeek3・4で読み返す（次項参照） |
| 『プロを目指す人のためのTypeScript入門』（通称ブルーベリー本） | 型の扱いを深く | 詰まった時の辞書。**O'Reillyサブスクを契約している場合は、代わりに『プログラミングTypeScript』(Boris Cherny著)または『初めてのTypeScript』(Josh Goldberg著、共にオライリー・ジャパンの日本語版)を通読してもよい。同格の体系書として代替可能** |
| 『実践Node.js入門』 | Node.js/Express全体像 | 詰まった時の辞書 |
| 『オブジェクト設計スタイルガイド』 | オブジェクト設計の実践知識 | Week2でRepository/Usecase設計に迷った時 |
| **『Pythonによるアーキテクチャパターン』**（Percival & Gregory著、O'Reilly、2025年邦訳） | Repositoryパターン・依存性逆転をゼロから構築する実例。Week2の内容に最も直結 | Week2着手前、またはRepository導入で迷った時。O'Reilly MCP/サブスクで参照可 |
| React 19の新機能まるわかり（Zenn本、無料） | React18→19の差分 | Week3 Day3の30分キャッチアップ |
| 『AWS Lambda実践ガイド 第2版』 | サーバーレスの基礎 | Week4 Day2着手前に該当章だけ |

**『SQLアンチパターン 第2版』の各章とこのプロジェクトの対応**（Week1着手前に軽く目を通した後、該当Dayで思い出す用）:

| 章 | アンチパターン | 対応するタイミング |
|---|---|---|
| 13章 インデックスショットガン | 根拠なくインデックスを張る/張らない | Week3 Day1。13.5節の「MENTOR」の原則（Measure/Explain/Nominate/Test/Optimize/Rebuild）をEXPLAIN演習の手順として使える |
| 14章 フィア・オブ・ジ・アンノウン | NULLの扱い誤り（`NOT IN`にNULL混入など） | Week4 Day1の集計SQLで「未回答の生徒/問題」を含めようとした時に踏みやすい罠 |
| 15章 アンビギュアスグループ | GROUP BYで集約していない列を参照 | Week4 Day1の集計SQL実装時。PostgresはMySQLと異なり厳格にエラーを返す点を比較すると理解が深まる。15.5.2のウィンドウ関数はWeek4 Day4のリファクタリング候補 |
| 21章 SQLインジェクション | 動的SQLの組み立てミス | Week3〜4で`$queryRaw`を書く際。タグ付きテンプレート(`` $queryRaw`...${value}` ``)を使えば基本回避できるが、21.6のミニ・アンチパターン（引用符内のパラメータ）は要注意 |
| 第V部（26〜27章）外部キーのミニ・アンチパターン | 標準SQL/MySQLでの誤ったFK定義 | 普段はPrismaの`@relation`が生成してくれるので意識しないが、マイグレーションでハマった時の辞書として |
...
**O'Reilly MCPの実際の機能について（重要）**: このMCPはカタログ検索（タイトル・著者・出版社などのメタデータ検索）のみで、**本文の全文検索はできない**。実装コードやその場の疑問への直接回答は返ってこないため、Claude Codeはこれを「質問に答えるツール」ではなく「次に開くべき信頼できる本を一瞬で見つける目利きツール」として案内すること。

**MCPの正しい使い方**:
1. 新しいトピックに入る直前に、信頼できる本の候補を探す（例: Week3のSQL/N+1のテーマに入る前）
2. Claude Codeが出した説明を「本当に一般的なやり方か」裏取りしたい時に、権威ある本がヒットするか確認する
3. 生成AI・アーキテクチャなど新しい分野を俯瞰したい時に一覧で見渡す

**MCPが向かない使い方**: 45分ルールで詰まった瞬間の一次対応（本文が取れないため時間がかかる）。この場合はまずClaude Codeが直接説明・実装例を示し、MCPは「その説明の裏取り」としてのみ使う。

**特にO'Reilly MCPでの本探しが効く場面**:
- Week2: Repository/Usecaseのインターフェース設計に迷った時（『オブジェクト設計スタイルガイド』『Pythonによるアーキテクチャパターン』はいずれもO'Reilly刊のため見つかりやすい）
- Week3 Day1〜2: `EXPLAIN`の実行計画の読み方、N+1問題、`$transaction`など、より深く学びたい時に該当書籍を探す
- Week4 Day1: 集計SQL（GROUP BY等）のインデックス設計をさらに深めたい時
- Week4 Day2: Lambdaのコールドスタート問題やAPI Gateway連携をさらに深めたい時

**逆に使わない方がよい場面**: Week1の「あえて汚い設計で進める」フェーズ。ここでMCPに頼ると最初から綺麗な設計に寄ってしまい、Week2のBefore/After体験が薄まるため、Claude Codeはこの期間はMCP参照よりまず自分で書かせることを優先する。

**概念を本文レベルで深く理解したい時の3段階フロー**: 単純な実装の疑問は45分ルールでClaude Codeが直接答えるが、「概念そのものを正確に理解したい」時（例: 依存性逆転の原則の背景など）は以下の3段階を踏む。

1. **MCPで本を絞り込む**（`search_oreilly_content`はメタデータ検索のみで本文は取得できないため、あくまで「どの本を見るべきか」の目利きとして使う）
2. **Claude in Chromeで実際にページを開いて要約させる**（Claude Codeと連携しており、ターミナルを離れずにブラウザでO'Reillyのページを開いて読ませられる。Pro以上の全プランで利用可能）
3. **複数ソースで正確性を確認する**: Claude in Chromeの要約とClaude Codeの説明を突き合わせ、食い違いがあればMCPで見つけた別の候補本も開いて三点比較する

**注意**: ブラウザ操作は通常のチャットより利用量の消費が大きいため、この3段階フローは「本当に深く理解したい概念」に限定して使い、実装の細かい疑問には使わないこと。
